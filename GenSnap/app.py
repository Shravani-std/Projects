...
from services.remove_video_bg import remove_video_background, check_video_processing_status

# Configure Streamlit page
st.set_page_config(
    page_title="GenSnap Studio",
    page_icon="🎨",
    layout="wide",
...
# Video Background Removal Tab
    with tabs[4]:   
        st.header("🎥 Remove Video Background")
        st.markdown("Upload a video or provide a URL to remove the background.")
        
        # Options for video input
        input_method = st.radio("Video Input Method", ["Upload Video", "Video URL"])
        
        if input_method == "Upload Video":
            uploaded_file = st.file_uploader("Upload Video", type=["mp4", "mov", "avi"], key="video_upload")
            video_url = None
        else:
            uploaded_file = None
            video_url = st.text_input("Enter Video URL", placeholder="https://example.com/input_video.mp4")
        ...
        if uploaded_file or video_url:
            ...
            with col1:
                ...
                content_moderation = st.checkbox("Enable Content Moderation", False, key="video_content_mod")
                resolution = st.selectbox("Output Resolution", ["Original", "720", "1080", "4K"], index=1)
                frame_rate = st.selectbox("Frame Rate", ["Original", "24", "30", "60"], index=2)
                        ...
                        with st.spinner("Processing video (this may take several minutes)..."):
                            try:
                                ...
                                result = remove_video_background(
                                    api_key=st.session_state.api_key,
                                    video_data=uploaded_file.getvalue() if uploaded_file else None,
                                    video_url=video_url if not uploaded_file else None,
                                    resolution=resolution if resolution != "Original" else None,
                                    frame_rate=frame_rate if frame_rate != "Original" else None,
                                    content_moderation=content_moderation
                                )
                                ...
                                if result:
                                    ...
                                    # Store the result information in session state
                                    if "result_url" in result:
                                        st.session_state.video_result_url = result["result_url"]
                                        st.session_state.video_processing_id = result.get("processing_id")
                                    elif "url" in result:
                                        st.session_state.video_result_url = result["url"]
                                        st.session_state.video_processing_id = result.get("processing_id")
                                    
                                    # If we got a direct result URL
                                    if st.session_state.video_result_url:
                                        st.info("Processing complete! Your video is ready.")
                                    
                            except Exception as e:
                                st.error(f"Error processing video: {str(e)}")
                                if "422" in str(e):
                                    st.warning("Content moderation failed. Please ensure the video is appropriate.")
            ...
            with col2:
                if 'video_result_url' in st.session_state and st.session_state.video_result_url:
                    ...
                    st.download_button(
                        ...
                        file_name="video_no_bg.mp4",
                        mime="video/mp4"
                    )
                else:
                    st.info("Processed video will appear here when ready.")
                    
                    # Status checking for async processing
                    if 'video_processing_id' in st.session_state and st.session_state.video_processing_id:
                        if st.button("Check Processing Status"):
                            with st.spinner("Checking status..."):
                                try:
                                    status_result = check_video_processing_status(
                                        api_key=st.session_state.api_key,
                                        processing_id=st.session_state.video_processing_id
                                    )
                                    
                                    if status_result:
                                        if status_result.get("status") == "completed":
                                            st.session_state.video_result_url = status_result.get("result_url")
                                            st.success("Processing complete!")
                                            st.experimental_rerun()
                                        else:
                                            st.info(f"Status: {status_result.get('status')}")
                                            st.info(f"Progress: {status_result.get('progress', 0)}%")
